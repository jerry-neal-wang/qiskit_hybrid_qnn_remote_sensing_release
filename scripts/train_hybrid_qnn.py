#!/usr/bin/env python3
"""Train the fusion-style CNN + QNN hybrid classifier for DIOR ROI data."""
...
try:
    ...
    from src.models.hybrid_model import HybridFusionQNNClassifier
    from src.models.qnn_head import get_qnn_dependency_report, qnn_dependencies_available
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train a fusion-style CNN + QNN hybrid classifier on DIOR ROI data."
    )
    parser.add_argument(
        "--dataset-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="CNN backbone feature width before the hybrid fusion head.",
    )
    parser.add_argument(
        "--backbone-type",
        type=str,
        ...
        help="Backbone family used by the hybrid model.",
    )
    parser.add_argument(
        "--pretrained-backbone",
        action="store_true",
        help="Initialize the hybrid backbone from torchvision pretrained weights when supported.",
    )
    parser.add_argument(
        "--input-normalization",
        type=str,
    ...
    parser.add_argument("--num-qubits", type=int, default=6, help="Number of qubits in the QNN head.")
    parser.add_argument(
        "--qnn-output-dim",
        type=int,
        default=4,
        help="Number of observables / output features produced by the QNN head.",
    )
    parser.add_argument(
        "--feature-map-reps",
        type=int,
        default=1,
        help="Repetitions for the ZZFeatureMap.",
    )
    parser.add_argument(
        "--ansatz-reps",
        type=int,
        default=1,
        help="Repetitions for the RealAmplitudes ansatz.",
    )
    parser.add_argument(
        "--circuit-type",
        type=str,
        default="zz_real_amplitudes",
        choices=("zz_real_amplitudes", "no_entanglement", "ry_ring"),
        help="Quantum circuit family used in the QNN head.",
    )
    parser.add_argument(
        "--fusion-mode",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Weight applied to the QNN branch contribution.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Observable construction strategy for EstimatorQNN.",
    )
    parser.add_argument(
        "--max-zz-observables",
        type=int,
        default=None,
        help="Optional cap on ZZ observables when qnn_output_dim exceeds the number of qubits.",
    )
    parser.add_argument(
...
def validate_environment(
    ...
    if IMPORT_ERROR is not None:
        raise ImportError(
            "PyTorch, qiskit-machine-learning, and local recognition modules are required "
            "for train_hybrid_qnn.py."
        ) from IMPORT_ERROR
    if not qnn_dependencies_available():
        dependency_report = get_qnn_dependency_report()
        missing_packages = dependency_report["missing_packages"]
        missing_symbols = dependency_report["missing_symbols"]
        install_commands = dependency_report["install_commands"]
        import_error = dependency_report["import_error"]

        missing_parts: list[str] = []
        if missing_packages:
            missing_parts.append("missing packages: " + ", ".join(str(item) for item in missing_packages))
        if missing_symbols:
            missing_parts.append("missing symbols: " + ", ".join(str(item) for item in missing_symbols))
        if import_error:
            missing_parts.append(f"import detail: {import_error}")

        raise ImportError(
            "Hybrid QNN dependencies are unavailable; "
            + "; ".join(missing_parts)
            + ". Install with: "
            + " or ".join(str(command) for command in install_commands)
            + "."
        )
    if pretrained_cnn_checkpoint is None and not allow_random_backbone_init:
        raise ValueError(
            "Hybrid training now requires an explicit --pretrained-cnn-checkpoint for fair comparison. "
...
def initialize_backbone_from_baseline(
    model: HybridFusionQNNClassifier,
    checkpoint_path: Path | None,
...
def set_backbone_trainable(model: HybridFusionQNNClassifier, trainable: bool) -> None:
...
def main() -> int:
    ...
    model = HybridFusionQNNClassifier(
        ...
        qnn_output_dim=args.qnn_output_dim,
        ...
        qnn_head_weight=args.qnn_head_weight,
        observable_mode=args.observable_mode,
        max_zz_observables=args.max_zz_observables,
    )
    ...
    plot_training_curves(history, curve_path, title="Hybrid QNN Training Curves")
    save_confusion_matrix_figure(
        test_metrics["confusion_matrix"],
        confusion_path,
        title="Hybrid QNN Confusion Matrix",
        labels=class_names,
    )
    ...
    config_payload = {
        ...
        "environment": collect_environment_info(
            ("numpy", "torch", "qiskit", "qiskit_machine_learning", "sklearn")
        ),
    }
