...
def draw_subset_montage(
    ...
    *,
    ...
    header_axis.text(
        0.0,
        0.25,
        "Representative test crops for the four-class subset used in the manuscript.",
        ha="left",
        va="bottom",
        fontsize=7.8,
        color=TEXT_MUTED,
...
def draw_pipeline_schematic(ax: plt.Axes) -> None:
    ...
    draw_box(
        ...
        0.26,
        "DIOR ROI subsets",
        "transport_logistics4 and\nurban_structural4 candidate subsets",
        "#F5F7FA",
    )
    draw_box(
        0.25,
    ...
    draw_box(
        ...
        0.16,
        "Bounded evaluation",
        "macro-F1, paired tests,\nbootstrap and runtime tradeoff",
        "#FFF7E8",
    )
    ...
    ax.text(
        0.02,
        0.08,
        "The figure isolates where any gain arises: same task, same subset, same backbone family, "
        "and a direct quantum-vs-classical head comparison before boundary stress tests.",
        transform=ax.transAxes,
        ha="left",
        va="bottom",
        fontsize=8.4,
...
def draw_boundary_protocol(ax: plt.Axes) -> None:
    """Draw the boundary-analysis summary table."""
    ax.axis("off")
    ax.text(
        0.0,
        1.02,
    ...
    rows = [
        ("Main window", "transport_logistics4", "Small CNN", "16", "7", "Positive low-shot test."),
        ("Shot boundary", "transport_logistics4", "Small CNN", "32 / 64", "7", "More labels, same subset."),
        ("Backbone boundary", "transport_logistics4", "ResNet18", "16", "7", "Stronger transfer features."),
        ("Subset boundary", "urban_structural4", "Small CNN", "16", "7", "Different subset composition."),
    ]
    ...
    row_facecolors = ["#F9FBFD", "#FFFFFF"]
    for row_index, row in enumerate(rows):
        y_bottom = y_top - (row_index + 1) * row_height
        x_cursor = 0.0
        for (value, (_, width)) in zip(row, columns):
            ax.add_patch(
                Rectangle(
                    ...
                    row_height - 0.01,
                    transform=ax.transAxes,
                    facecolor=row_facecolors[row_index % 2],
                    edgecolor="#E5E9EF",
                    linewidth=0.7,
                )
            )
            ax.text(
                x_cursor + 0.01,
                y_bottom + (row_height - 0.01) / 2.0,
                wrap_text(value, 18 if width > 0.2 else 11),
                transform=ax.transAxes,
                ha="left",
                va="center",
                fontsize=8.2,
    ...
    ax.text(
        0.0,
        0.06,
        "The paper's main claim is visible here as a test ladder rather than a single isolated win.",
        transform=ax.transAxes,
        ha="left",
        va="bottom",
        fontsize=8.4,
...
def plot_paired_delta_panel(
    ...
    for index, method in enumerate(QNN_METHODS):
        ...
        ci_low, ci_high, prob_gt_zero = bootstrap_ci(deltas, seed=index + 10)
    ...
    for condition in CONDITIONS:
        ...
        for method in QNN_METHODS:
            ...
            ci_low, ci_high, prob_gt_zero = bootstrap_ci(
                deltas,
                seed=hash((condition.condition_id, method)) % (2**32),
            )
            records.append(
                {
...
def plot_boundary_heatmap(ax: plt.Axes, records: list[dict[str, Any]]) -> None:
    ...
    ax.set_title("Where the quantum-control margin appears")
...
def plot_transport_shot_trajectory(ax: plt.Axes) -> None:
    ...
    ax.set_xlabel("Shot")
    ax.set_ylabel("Macro-F1")
    ax.set_title("Transport subset: gain contracts with more labels")
    ax.grid(True, axis="y")
...
def plot_bootstrap_forest(ax: plt.Axes, records: list[dict[str, Any]]) -> None:
    ...
    ax.set_yticklabels(
        [
            f"{record['condition_short_label'].replace(chr(10), ' / ')} / {QNN_LABELS[record['method']]}"
            for record in plot_records
        ]
    )
    ax.set_xlabel("Bootstrap 95% CI of macro-F1 delta vs control")
    ax.set_title("Confidence intervals expose the regime boundary")
    ax.grid(True, axis="x")
...
def plot_efficiency_tradeoff(ax: plt.Axes, records: list[dict[str, Any]]) -> None:
    ...
    for method in QNN_METHODS:
        ...
        for record in method_records:
            ax.text(
                float(record["runtime_ratio_vs_control"]) + 0.03,
                float(record["mean_delta"]),
                f"{record['subset_name'].split('_')[0]}-{record['shot']}",
                fontsize=7.4,
                color=TEXT_DARK,
                ha="left",
                va="center",
    ...
    ax.text(1.02, -0.085, "control runtime", fontsize=7.6, color=TEXT_MUTED, ha="left", va="bottom")
    ax.set_xlabel("Runtime multiplier vs control")
    ax.set_ylabel("Macro-F1 delta vs control")
    ax.set_title("Practical boundary: cost rises faster than the margin")
    ax.grid(True)
...
def draw_figure_1(output_dir: Path) -> list[Path]:
    ...
    draw_subset_montage(
        ...
        class_order=["airplane", "ship", "vehicle", "harbor"],
        title="Transport subset used for the main low-shot claim",
    ...
    draw_subset_montage(
        ...
        title="Urban subset used to test cross-subset transfer",
        counts=urban_counts,
        panel_label="b",
    )
